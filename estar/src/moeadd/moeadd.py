...
class moeadd_solution(object):
    def __init__(self, x, obj_funs):
        self.vals = x #
        self.obj_funs = obj_funs # objectives - критерии, из Омега в число
        self.precomputed_value = False
        self.precomputed_domain = False
        # self.obj_fun - вектор F
...
#        print('Pareto levels update: before', [individ.vals for individ in  self.population])
#        print('lengths:', len(self.population), sum([len(level) for level in self.levels]))        
#        for level_idx, level in enumerate(self.levels):
#            print(level_idx, [individ.vals for individ in level])
        self.levels = self._update_method(point, self.levels)
        self.population.append(point)
#        print('\n')
#        print('-||- after', [individ.vals for individ in  self.population])
#        print('lengths:', len(self.population), sum([len(level) for level in self.levels]))
#        for level_idx, level in enumerate(self.levels):
#            print(level_idx, [individ.vals for individ in level])
#        print('\n')
...
class moeadd_optimizer(object):
    ...
    def __init__(self, pop_constructor, weights_num, pop_size, solution_params, delta, neighbors_number, 
        ...
        assert type(solution_params) == type(None) or type(solution_params) == dict, 'The individ parameters, passed into population constructor must be in dictionary'
        population = []
