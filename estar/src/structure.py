    ...
    def Randomize(self, forbidden_factors=None):
        ...
        while True:
            ...
            if selected_meaningful_family.status[
                'unique_specific_token']:  # 'single' - токен не может повторяться (даже с разными пар-ми)
                self.occupied_tokens_labels.append(selected_label)
            elif selected_meaningful_family.status[
                'unique_token_type']:  # 'unique_token' - в слагаемом не может быть 2 мн-теля выбранного типа
            ...
            self.structure.append(Factor(selected_label, selected_meaningful_family,
                                         randomize=True))  # Сделать возм. коллапсировать одинаковые множители в один с большей степенью
            ...
            for i in np.arange(1, factors_num):
                ...
                if selected_token.status[
                    'unique_specific_token']:  # 'unique_token_type' - токен не может повторяться (даже с разными пар-ми)
                    self.occupied_tokens_labels.append(selected_label)
                elif selected_token.status[
                    'unique_token_type']:  # 'unique_specific_token' - в слагаемом не может быть 2 мн-теля выбранного типа
        ...
        if self.saved[normalize]:
            ...
            if np.all([len(factor.params) == 1 for factor in self.structure]):
                ...
                self.saved[normalize] = global_var.tensor_cache.add(self.cache_label, value,
                                                                    normalize)  # Место возможных проблем: сохранение/загрузка нормализованных данных
